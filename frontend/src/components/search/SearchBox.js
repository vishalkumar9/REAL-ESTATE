...
const SearchBox = () => {
    ...
    try {
      if (city || location || type || propertyType || maxPrice) {
        ...
        const response = await fetch(
          `${process.env.REACT_APP_BACKEND_PROPERTY_URL}/searchProperty`,
          {
            method: 'POST',
            headers: {
  ...
  useEffect(() => {
    const fetchAvailableLocations = async () => {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_BACKEND_PROPERTY_URL}/getAvailableLocations`,
        )
        const responseData = await response.json()
    ...
    <div className="search_box">
      ...
      <div className="input_main_div">
        ...
        <ShowOption
          ...
          onChange={(option) => setMaxPrice(option)}
          text="max Price"
          flag={false}
        />
        {
          <button
            className="active"
            onClick={AuthC.token ? handleSubmit : handleError}
          >
